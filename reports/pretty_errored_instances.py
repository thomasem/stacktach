    ...
    for uuid_dict in updates:
        ...
        for req_dict in reqs:
            ...
            for raw in raws:
                ...
                if tenant == "other" and raw.tenant:
                    tenant = raw.tenant
            ...
            if failure_type:
                ...
                if failure_type == "5xx":
                    failures_5xx[tenant] = failures_5xx.get(tenant, {})
                    failures_5xx[tenant][os_type] = failures_5xx[tenant].get(os_type, [])
                    failures_5xx[tenant][os_type].append(uuid)
                if failure_type == "state":
                    failures_error[tenant] = failures_error.get(tenant, {})
                    failures_error[tenant][os_type] = failures_error[tenant].get(os_type, [])
                    failures_error[tenant][os_type].append(uuid)
                if failure_type == too_long_col:
                    failures_30_min[tenant] = failures_30_min.get(tenant, {})
                    failures_30_min[tenant][os_type] = failures_30_min[tenant].get(os_type, [])
                    failures_30_min[tenant][os_type].append(uuid)

    print "----------- 5xx -----------"
    print json.dumps(failures_5xx, indent=4)
    print "----------- error state -----------"
    print json.dumps(failures_error, indent=4)
    print "----------- >30 min -----------"
    print json.dumps(failures_30_min, indent=4)
